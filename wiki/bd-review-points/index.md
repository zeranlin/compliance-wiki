...
- 当前已覆盖 153 个 NBD：`maintained` 153 个，`testing` 0 个，`draft` 0 个。
...
- 2026-04-30 已完成 NBD 瘦身与 IR 拆分治理：153 个 maintained NBD 均可编译出 NBD IR、Recall IR、Prompt IR、Governance IR。
- 日常 CLI prompt 已切换为读取 Prompt IR 可执行 SOP，不再读取整页 NBD markdown；0044-A P4 回归 153 个模型结果，失败数 0。
...
## 建设计划
- 当前计划数量：153 个 NBD 执行单元，约 151 个审查点页面。
