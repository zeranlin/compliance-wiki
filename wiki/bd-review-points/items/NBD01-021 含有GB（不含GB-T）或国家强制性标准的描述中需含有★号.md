---
id: NBD01-021
title: 含有GB（不含GB/T）或国家强制性标准的描述中需含有★号
...
# NBD01-021 含有GB（不含GB/T）或国家强制性标准的描述中需含有★号

## 审查目标
在采购文件中检索所有强制性描述或强制性标准（编号前缀为GB,不包含GB/T),检查引用了强制性标准的条款描述是否标注了“★”号标识。若没有“★”号标识，则存在风险，并进行提示。

注：若条款中同时出现国家强制性标准和非强制性标准时，仍然需要有“★”号标识，并进行提示。
...
### 数值/模式规则
- 国务院已明令取消.*(资格|评分|合同|需求|无效|不通过)
- 已取消.*(资格|评分|合同|需求|无效|不通过)
- 非强制.*(资格|评分|合同|需求|无效|不通过)
- 国家行政机关非强制.*(资格|评分|合同|需求|无效|不通过)
...
## 命中条件
同时满足以下条件时，输出命中：
- 正式条款中出现与“国务院已明令取消、已取消、非强制、国家行政机关非强制”相关的审查对象。
...
## 风险提示
条款中引用的“{标准名称及编号}”属于国家强制性标准，但未按照规定标注“★”号标识，可能导致合规性风险。

## 修改建议
请核实此处检测结果要求是否和标准一致？如一致，请设置为★实质性条款。
