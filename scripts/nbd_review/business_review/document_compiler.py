...
from shared.schemas import DOCUMENT_IR_SCHEMA, DocumentBlock, STRUCTURE_PATTERNS
from shared.utils import compact, looks_like_heading, read_text, run_path, vcc, write_text
...
def _score_mentions(text: str) -> list[dict[str, Any]]:
    patterns = [
        ("item_max", r"本小项最高得?\s*(\d+(?:\.\d+)?)\s*分"),
        ("group_max", r"本项最高得?\s*(\d+(?:\.\d+)?)\s*分"),
        ("total_max", r"(?:累计)?最高得?\s*(\d+(?:\.\d+)?)\s*分"),
        ("score", r"(?:得|加)\s*(\d+(?:\.\d+)?)\s*分"),
    ]
    ...
    for kind, pattern in patterns:
        for match in re.finditer(pattern, text):
            try:
                ...
                continue
            mentions.append({"kind": kind, "value": value, "raw": match.group(0)})
    return mentions


def row_scoring_analysis(row: list[str], row_number: int) -> dict[str, Any]:
    text = " ".join(cell for cell in row if cell)
    ctext = compact(text)
    if not re.search(r"得\s*\d+(?:\.\d+)?\s*分|加\s*\d+(?:\.\d+)?\s*分|最高得?\s*\d+(?:\.\d+)?\s*分", text):
        return {}
    if any(word in ctext for word in ["序号评分因素权重", "评分准则", "价格分计算方法"]):
        return {}
    non_empty = [(idx, cell) for idx, cell in enumerate(row) if cell]
    if not non_empty:
        return {}
    longest_idx, longest_text = max(non_empty, key=lambda item: len(item[1]))
    numeric_before = [
        {"column_index": idx, "value": value, "raw": cell}
        for idx, cell in non_empty
        if idx < longest_idx and (value := _number_value(cell)) is not None
    ]
    declared_weight = numeric_before[-1] if numeric_before else None
    label = ""
    if declared_weight:
        for idx, cell in reversed(non_empty):
            if idx >= int(declared_weight["column_index"]):
                continue
            if _number_value(cell) is None and len(compact(cell)) > 1:
                label = cell
                break
    mentions = _score_mentions(longest_text or text)
    max_internal = max((mention["value"] for mention in mentions), default=None)
    return {
        "row_number": row_number,
        "label": label,
        "declared_weight": declared_weight,
        "score_mentions": mentions[:20],
        "max_internal_score": max_internal,
        # Many scoring tables use an internal 100-point row score that is later
        # folded into a weight column. Treat that as neutral structure, not an
        # anomaly, so downstream prompts do not invent a weight-conflict issue.
        "anomaly": {},
    }
...
def table_ir_from_rows(rows: list[list[str]]) -> dict[str, Any]:
    ...
    row_analyses = [row_scoring_analysis(row, offset) for offset, row in enumerate(normalized_rows, start=1)]
        ...
        scoring_rows.append(
            {
                ...
                "row_scoring_analysis": row_analyses[offset - 1] if offset - 1 < len(row_analyses) else {},
                "cells": row,
            }
        )
    top_weights = [
    ...
    return {
        ...
        "scoring": {
            ...
            "row_analyses": [analysis for analysis in row_analyses if analysis][:80],
...
def extract_docx_blocks(path: Path) -> tuple[list[DocumentBlock], dict[str, Any], str]:
    ...
    def xml_visible_text(element: Any) -> str:
        parts: list[str] = []
        for node in element.iter():
            ...
            if local_name == "t" and node.text:
                ...
                parts.append("\n")
        return vcc.normalize_extracted_text("".join(parts)).strip()
    ...
    def table_rows(table: Any) -> list[list[str]]:
        ...
        for row in table.rows:
            cells = [
                re.sub(r"\s+", " ", xml_visible_text(cell._tc)).strip()
                for cell in row.cells
            ]
            non_empty = [cell for cell in cells if cell]
            unique_non_empty = set(non_empty)
            if len(unique_non_empty) == 1 and re.fullmatch(r"\d+", compact(non_empty[0])):
                continue
            sparse_key = "\t".join(compact(cell) for cell in non_empty)
            if len(non_empty) <= 2 and sparse_key and (
                sparse_key == previous_sparse_key or previous_sparse_key.startswith(f"{sparse_key}\t")
            ):
                continue
            if any(cells):
                rows.append(cells)
                previous_sparse_key = sparse_key
        return rows
    ...
    block_index = 1
    table_index = 1
    stats = {"paragraph_blocks": 0, "table_row_blocks": 0, "table_count": 0, "block_count": 0, "line_count": 0}
    for item in iter_block_items(document):
        if item.__class__.__name__ == "Paragraph":
            text = xml_visible_text(item._p)
            if not text:
                continue
            blocks.append(
                DocumentBlock(
            ...
            if not rows:
                continue
            table_ir = table_ir_from_rows(rows)
            for row_number, row in enumerate(rows, start=1):
                text = table_row_to_text(row)
                if not text:
                    continue
                row_analysis = {}
                scoring = table_ir.get("scoring") or {}
                for analysis in scoring.get("row_analyses") or []:
                    if analysis.get("row_number") == row_number:
                ...
                blocks.append(
                    DocumentBlock(
                        ...
                        table={
                            ...
                            "full_table": table_ir if row_number == 1 else {},
                        },
                    )
                )
                stats["table_row_blocks"] += 1
                line_no += 1
                block_index += 1
            stats["table_count"] += 1
            table_index += 1
    ...
    return blocks, stats, "python-docx-word-body-lines"
...
def infer_section_role(text: str, current_role: str) -> tuple[str, float, list[str]]:
    ...
    support_roles = {"catalog", "common_terms", "bid_format", "contract_template", "template_support", "policy_support"}
    if (
        "\t" in text
        and re.search(r"得\d+(?:\.\d+)?分|不得分|加\d+(?:\.\d+)?分|专家打分|评分", ctext)
        and re.search(r"（示例）|\(示例\)|（可选）|\(可选\)|可选，仅适用于|提供\d+个xx", text)
    ):
        return "template_support", 0.82, ["命中示例/可选评分模板行"]
    if (
        "\t" in text
        and re.search(r"得\d+(?:\.\d+)?分|不得分|加\d+(?:\.\d+)?分|本项最高|本小项最高|权重", ctext)
        and re.search(r"考察内容|评分|评审|证明文件|证明材料|评分因素", ctext)
    ):
    ...
    if (
        re.search(r"(供应商|投标人).{0,18}(须|必须|应|需).{0,60}(资格|法人|营业执照|独立承担民事责任|证照)", ctext)
        and not re.search(r"得\d*(?:\.\d+)?分|不得分|评分|评审|加分|扣分", ctext)
    ):
        return "qualification_primary", 0.7, ["命中供应商/投标人资格约束句式"]
    best_role = current_role or "unknown"
    best_conf = 0.45 if current_role in primary_roles else 0.2 if current_role and current_role != "unknown" else 0.0
    ...
    for role, title_words, table_words in STRUCTURE_PATTERNS:
        ...
        if role == "catalog" and len(text) > 120:
            conf -= 0.25
        if role in support_roles and current_role in primary_roles and len(ctext) > 80 and not looks_like_heading(text.strip()):
            conf -= 0.35
        if conf > best_conf:
            best_role = role
        ...
        scoring_tables.append(
            {
                ...
                "row_analyses": [
                    analysis
                    for analysis in (scoring.get("row_analyses") or [])[:20]
                ...
                "rows": [
                    {
                        ...
                        "row_scoring_analysis": row.get("row_scoring_analysis") or {},
                    }
...
def fact_summary_markdown(facts: dict[str, Any]) -> str:
    scoring_tables = facts.get("scoring_tables", [])
    return "\n".join(
        [
            "## 轻事实摘要",
            "",
            ...
            f"- 评分表数量：{len(scoring_tables) if isinstance(scoring_tables, list) else 0}",
        ]
    )
