...
from business_review.recall_runner import write_recall_matrix
from shared.utils import now_text, read_text, relative_path, write_text
...
def write_report_artifacts(output_dir: Path, results: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    ...
    for row in results:
        ...
        prune_support_positive_candidates(row)
        flags = model_quality_flags(row)
        row["quality_flags"] = flags
        if isinstance(row.get("model_result"), dict):
            row["model_result"]["quality_flags"] = flags
        nbd_id = str((row.get("nbd") or {}).get("id") or (row.get("model_result") or {}).get("nbd_id") or "")
        if nbd_id:
            result_file = output_dir / "items" / nbd_id / "result.json"
            if result_file.exists():
                write_text(result_file, json.dumps(row, ensure_ascii=False, indent=2) + "\n")
